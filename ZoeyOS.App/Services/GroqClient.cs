...
{
    /// <summary>
    /// Thin wrapper around Groq's free-tier API (api.groq.com), which speaks the
    /// standard OpenAI chat-completions format. No billing/credit card required.
    /// Faster and has a more generous permanent free tier than Gemini, but doesn't
    /// bundle image generation the way Gemini does.
    /// </summary>
    public class GroqClient : IChatEngine
    {
        ...
        /// Whisper (audio) and Orpheus (voice) models. Unfiltered on purpose, same
        /// reasoning as OpenAIClient.ListModelsAsync.</summary>
        public async Task<List<string>> ListModelsAsync()
        {
            if (!IsConfigured) return new List<string>();

            var response = await _http.GetAsync("https://api.groq.com/openai/v1/models");
            var text = await response.Content.ReadAsStringAsync();
            if (!response.IsSuccessStatusCode)
                throw new InvalidOperationException($"Groq returned {(int)response.StatusCode}: {text}");
        ...
        /// <summary>Sends the companion's full history plus its system prompt, returns Groq's reply text.</summary>
        public async Task<string> SendAsync(string systemPrompt, IEnumerable<ChatMessage> history, string newUserMessage)
        {
            if (!IsConfigured)
                return "[No Groq API key set. Add one in Settings to bring this companion online.]";
            ...
            try
            {
                ...
                if (!response.IsSuccessStatusCode)
                    return $"[Groq API error {(int)response.StatusCode}: {responseText}]";
                ...
                foreach (var choice in choices.EnumerateArray())
                {
                    ...
                    if (message.TryGetProperty("content", out var c) && c.ValueKind == JsonValueKind.String)
                        return c.GetString() ?? "[Groq returned an empty response.]";
                }
                return "[Groq returned an empty response.]";
            }
            catch (Exception ex)
            {
                return $"[Connection error reaching Groq: {ex.Message}]";
            }
        }

        /// <summary>Converts our Anthropic-shaped tool schema (name/description/input_schema) into
        /// OpenAI/Groq's function-tool shape (type/function{name,description,parameters}).</summary>
        ...
        /// <summary>
        /// Same as SendAsync, but lets Groq call tools (e.g. smart home control) mid-turn.
        /// Loops: send -> if Groq asks for a tool, run it via executeTool and feed the result
        /// back -> repeat until Groq gives a final text answer or the turn limit is hit.
        /// </summary>
            ...
            for (int turn = 0; turn < maxToolTurns; turn++)
            {
                ...
                if (!message.TryGetProperty("tool_calls", out var toolCalls) || toolCalls.GetArrayLength() == 0)
                {
                    var text = message.TryGetProperty("content", out var c) && c.ValueKind == JsonValueKind.String
                        ? c.GetString()
                        : null;
                    return string.IsNullOrEmpty(text) ? "[Groq returned an empty response.]" : text;
                }
